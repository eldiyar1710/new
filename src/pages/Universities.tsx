...
import { Search, Filter, Heart, Star, MapPin, Users, DollarSign, Calendar, BookOpen, Lock, Check } from 'lucide-react';
...
import { 
  ...
  getUniversity, 
  searchUniversityAPI, 
  getUniversitiesByCountryAPI,
  getPremiumUniversitiesAPI,
  addToFavorites,
  removeFromFavorites,
  getFavorites,
  University 
    ...
    try {
      if (favorites.includes(universityId)) {
        ...
        toast({
          ...
          description: "Университет удален из вашего списка избранного"
        });
      } else {
        ...
        toast({
          ...
          description: "Университет добавлен в ваш список избранного"
        });
      }
    } catch (error) {
      toast({
  ...
  const viewUniversityDetails = async (university: University) => {
    if (university.isPremium && !isAuthed()) {
      toast({
        ...
        description: "Для просмотра подробной информации необходимо авторизоваться и оплатить подписку",
        variant: "destructive"
      });
      return;
    }
